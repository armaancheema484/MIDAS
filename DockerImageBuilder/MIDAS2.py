...
if not args.base_image:
    print("\nAvailable set of base images for MIDAS: ")
    for index, image in enumerate(base_images, start=1):
        print(f"{index}. {image}")
    selection = input("\nPlease select the base image by entering its number: ")
...
# Processes warnings
warnings_to_check = {
                    "CMD": not args.ignore_cmd_warnings,
...
# SNYK Analysis
def run_snyk_analysis(image_name):

    image_name = client.images.build(path=docker_path_dir, dockerfile=args.output, tag=dockerhub_tag,
                                     timeout=args.timeout)

    subprocess.run(["snyk", "auth", "a713dc3e-62cf-4684-8e16-8e8b196bb98e"])

    output = subprocess.check_output(["snyk", "test", "--all-projects", "--json", "--docker", image_name])
